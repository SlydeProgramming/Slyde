...
import slyde.structure.AST.ASTNode;
import slyde.structure.AST.AssignmentNode;
import slyde.structure.AST.BinaryOpNode;
import slyde.structure.AST.BlockNode;
import slyde.structure.AST.BooleanNode;
import slyde.structure.AST.ClassNode;
import slyde.structure.AST.ConditionalOp;
import slyde.structure.AST.ConstructorNode;
import slyde.structure.AST.DoubleNode;
import slyde.structure.AST.ForNode;
import slyde.structure.AST.IdentifierNode;
import slyde.structure.AST.IfNode;
import slyde.structure.AST.MainNode;
import slyde.structure.AST.MethodCallNode;
import slyde.structure.AST.MethodNode;
import slyde.structure.AST.NewInstanceNode;
import slyde.structure.AST.NumberNode;
import slyde.structure.AST.ProgramNode;
import slyde.structure.AST.ReturnNode;
import slyde.structure.AST.StringNode;
import slyde.structure.AST.VarDeclNode;
import slyde.structure.AST.WhileNode;
...
public class LLVMGenerator {
  ...
  private void generateClass(ClassNode classNode) {
    ...
    for (VarDeclNode n : vars) {
      ...
      llvmCode.append(indent.get() + "%" + n.name + " = getelementptr %" + classNode.name + ", %" + classNode.name
          + "* %this, i32 0, i32 " + index + "\n");
      String[] valueRef = generateExpression(n.value, true);
      llvmCode.append(indent.get() + "store " + valueRef[1] + " " + valueRef[0] + ", " + getLLVMType(n.type)
          + "* %" + n.name + "\n");
      index++;
    }
    ...
    for (MethodNode m : methods) {
      llvmCode.append("define " + getLLVMType(m.returnType) + " @" + classNode.name + "_" + m.name + "(%"
          + classNode.name + "* %this");
      ...
      for (VarDeclNode n : vars) {
        ...
        llvmCode.append(indent.get() + "%" + n.name + " = getelementptr %" + classNode.name + ", %"
            + classNode.name + "* %this, i32 0, i32 " + index2 + "\n");
        index2++;
      }
  ...
  private void generateIf(IfNode ifNode) {
    ...
    llvmCode.append(
        indent.get() + "br i1 " + condition + ", label %" + trueLabel + ", label %" + falseLabel + "\n");
  ...
  private String[] createString(StringNode str, boolean getType) {
    ...
    llvmCode.append(
        indent.get() + ref + " = getelementptr " + type + ", " + type + "* " + temp + ", i32 0, i32 0\n");
    llvmCode.append(indent.get() + "store i8 " + (int) value.charAt(1) + ", i8* " + ref + "\n");

    for (int i = 2; i < len - 1; i++) {
      String newStringPtr = newStrVar();
      llvmCode.append(
          indent.get() + newStringPtr + " = getelementptr i8, i8* " + ref + ", i32 " + (i - 1) + "\n");
  ...
  private String generateCondOp(ConditionalOp conditionalOp) {
    ...
    llvmCode.append(
        indent.get() + temp + " = " + t + "cmp " + op + " " + left[1] + " " + left[0] + ", " + right + "\n");
